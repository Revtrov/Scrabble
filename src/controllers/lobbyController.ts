...
export const isPlayersTurn =(req: Request, res: Response) => {
  const lobbyId = req.params.lobbyId;
  const lobby = lobbyMap.get(lobbyId);
  if (!lobby) return res.status(404).json({ message: "Lobby not found" });
  ...
  const player = lobby.playerIdMap.get(playerId);
  if (!player) return res.status(404).json({ message: "Player not found" });
  return res.status(200).json(lobby.getGameManager().isPlayersTurn(player))
}

export const getTileBag = (req: Request, res: Response) => {
  const lobbyId = req.params.lobbyId;
  const lobby = lobbyMap.get(lobbyId);
  if (!lobby) return res.status(404).json({ message: "Lobby not found" });
  return res.status(200).json(lobby.getGameManager().getTileBag().asDTO())
}
